package net.lab1024.sa.support.job.core;
...
public class SmartJobLauncher {
  ...
  public SmartJobLauncher(
    ...
    this.launcherExecutor = new ScheduledThreadPoolExecutor(1, factory);
    ...
    this.launcherExecutor.scheduleWithFixedDelay(
        launcherRunnable, initDelay, refreshInterval, TimeUnit.SECONDS);

    // 打印信息
    String refreshDesc = refreshEnabled ? "开启|检测间隔" + refreshInterval + "秒" : "关闭";
    String format =
        String.format(SmartJobConst.LOGO, jobConfig.getCorePoolSize(), initDelay, refreshDesc);
  ...
  public void startOrRefreshJob(List<SmartJobEntity> smartJobList) {
    ...
    // 任务实现类
    Map<String, SmartJob> jobImplMap =
        jobInterfaceList.stream()
            .collect(Collectors.toMap(SmartJob::getClassName, Function.identity()));
  ...
  /**
   * 手动判断 任务配置 是否需要更新 新增字段的话 在这个方法里增加判断
   *
   * @return
   */
