...
/**
 * 定时任务 接口业务管理 如果不需要通过接口管理定时任务 可以删除此类
 *
 * @author huke
...
public class SmartJobService {
  ...
  private void handleJobInfo(List<SmartJobVO> jobList) {
    ...
    List<Long> logIdList =
        jobList.stream()
            .map(SmartJobVO::getLastExecuteLogId)
            .filter(Objects::nonNull)
            .collect(Collectors.toList());
    Map<Long, SmartJobLogVO> lastLogMap = Collections.emptyMap();
    if (CollectionUtils.isNotEmpty(logIdList)) {
      lastLogMap =
          jobLogDao.selectBatchIds(logIdList).stream()
              .collect(
                  Collectors.toMap(
                      SmartJobLogEntity::getLogId,
                      e -> SmartBeanUtil.copy(e, SmartJobLogVO.class)));
    }
  ...
  private ResponseDTO<String> checkParam(SmartJobAddForm addForm) {
    ...
    if (SmartJobTriggerTypeEnum.CRON.equalsValue(triggerType)
        && !SmartJobUtil.checkCron(triggerValue)) {
      return ResponseDTO.userErrorParam("cron表达式错误");
    }
    if (SmartJobTriggerTypeEnum.FIXED_DELAY.equalsValue(triggerType)
        && !SmartJobUtil.checkFixedDelay(triggerValue)) {
