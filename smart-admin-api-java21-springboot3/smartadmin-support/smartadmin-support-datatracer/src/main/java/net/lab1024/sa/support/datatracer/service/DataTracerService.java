...
import net.lab1024.sa.support.datatracer.manager.DataTracerManger;
...
public class DataTracerService {
  ...
  public void insert(Long dataId, DataTracerTypeEnum type) {
    DataTracerForm form =
        DataTracerForm.builder().dataId(dataId).type(type).content(DataTracerConst.INSERT).build();
    this.addTrace(form);
  }
  ...
  public void delete(Long dataId, DataTracerTypeEnum type) {
    DataTracerForm form =
        DataTracerForm.builder().dataId(dataId).type(type).content(DataTracerConst.DELETE).build();
    this.addTrace(form);
  }
  ...
  public void addTrace(Long dataId, DataTracerTypeEnum type, String content) {
    DataTracerForm form =
        DataTracerForm.builder().dataId(dataId).type(type).content(content).build();
    this.addTrace(form);
  }
  ...
  public void addTraceList(List<DataTracerForm> tracerFormList, RequestUser requestUser) {
    ...
    List<DataTracerEntity> tracerEntityList =
        tracerFormList.stream()
            .map(
                e -> {
