...
import net.lab1024.sa.common.core.util.SmartBigDecimalUtil;
...
import net.lab1024.sa.support.datatracer.annoation.DataTracerFieldBigDecimal;
import net.lab1024.sa.support.datatracer.annoation.DataTracerFieldDict;
...
import net.lab1024.sa.support.datatracer.annoation.DataTracerFieldSql;
import net.lab1024.sa.support.datatracer.constant.DataTracerConst;
import net.lab1024.sa.support.datatracer.domain.bo.DataTracerContentBO;
...
public class DataTracerChangeContentService {
  ...
  /** 类 加注解字段缓存 */
  private final Map<Class<?>, List<Field>> fieldMap = new ConcurrentHashMap<>();
  ...
  /**
   * 解析多个对象的变更，删除，新增 oldObject 为空 ，newObject 不为空 为新增 oldObject 不为空 ，newObject 不空 为删除 都不为空为编辑
   *
  ...
  private <T> String getUpdateContentList(List<T> oldObjectList, List<T> newObjectList) {
    ...
    return "【原数据】:" + HTML_BR + oldContent + HTML_BR + "【新数据】:" + HTML_BR + newContent;
  }
  ...
  private <T> String getObjectListContent(List<T> objectList) {
    ...
    return StringUtils.join(contentList, HTML_BR);
  }
  ...
  private String getAddDeleteContent(Map<String, DataTracerContentBO> beanParseMap) {
    ...
    for (Entry<String, DataTracerContentBO> entry : beanParseMap.entrySet()) {
      ...
      if (jsonFlag) {
        ...
        contentList.add(
            dataTracerContentBO.getFieldDesc() + ":" + dataTracerContentBO.getFieldContent());
      }
    }
    String operateContent = StringUtils.join(contentList, HTML_BR);
    if (StringUtils.isEmpty(operateContent)) {
      return "";
    }
    return operateContent;
  ...
  private <T> String getUpdateContent(T oldObject, T newObject) {
    ...
    for (Entry<String, DataTracerContentBO> entry : oldBeanParseMap.entrySet()) {
      ...
      // fieldContent
      String oldContent =
          oldContentBO == null || oldContentBO.getFieldContent() == null
              ? ""
              : oldContentBO.getFieldContent();
      String newContent =
          newContentBO == null || newContentBO.getFieldContent() == null
              ? ""
              : newContentBO.getFieldContent();
  ...
  private DataTracerContentBO getFieldValue(Field field, Object object) {
    ...
    if (dataTracerFieldEnum != null) {
      if (fieldValue instanceof Collection) {
        fieldContent =
            SmartEnumUtil.getEnumDescByValueList(
                (Collection) fieldValue, dataTracerFieldEnum.enumClass());
      } else {
        fieldContent =
            SmartEnumUtil.getEnumDescByValue(fieldValue, dataTracerFieldEnum.enumClass());
      }
    } else if (dataTracerFieldDict != null) {
      DictDataVO dictData =
          dictService.getDictData(dataTracerFieldDict.dictCode(), fieldValue.toString());
      ...
      DataTracerFieldBigDecimal dataTracerFieldBigDecimal =
          field.getAnnotation(DataTracerFieldBigDecimal.class);
      if (dataTracerFieldBigDecimal != null) {
        BigDecimal value =
            SmartBigDecimalUtil.setScale(
                (BigDecimal) fieldValue, dataTracerFieldBigDecimal.scale());
  ...
  private String getRelateDisplayValue(Object fieldValue, DataTracerFieldSql dataTracerFieldSql) {
    ...
    qw.select(
        CaseFormat.UPPER_CAMEL.to(
            CaseFormat.LOWER_UNDERSCORE, dataTracerFieldSql.relateDisplayColumn()));
    qw.in(
