...
import net.lab1024.sa.support.codegenerator.constant.CodeQueryFieldQueryTypeEnum;
  ...
  public Map<String, Object> getInjectVariablesMap(CodeGeneratorConfigForm form) {
    ...
    frontImportSet.add(
        "import "
            ...
            + "Form from './"
            + CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_HYPHEN, form.getBasic().getModuleName())
            + "-form.vue';");
    ...
    for (CodeQueryField queryField : queryFields) {
      ...
      if (CodeQueryFieldQueryTypeEnum.ENUM.equalsValue(queryField.getQueryTypeEnum())
          && SmartStringUtil.isNotBlank(codeField.getEnumName())) {
    ...
    List<CodeTableField> tableFields =
        form.getTableFields().stream().filter(CodeTableField::getShowFlag).toList();
    ...
    for (CodeTableField tableField : tableFields) {
      ...
      if (SmartStringUtil.isNotBlank(codeField.getEnumName())) {
        ...
        objectMap.put(
            "frontEnumPlugin",
            "$smartEnumPlugin.getDescByValue('" + upperUnderscoreEnum + "', text)");
      }
      ...
      CodeInsertAndUpdateField codeInsertAndUpdateField =
          form.getInsertAndUpdate().getFieldList().stream()
              .filter(e -> SmartStringUtil.equals(tableField.getColumnName(), e.getColumnName()))
              .findFirst()
              .orElse(null);
      ...
      // 是否存在上传组件
      if (CodeFrontComponentEnum.FILE_UPLOAD.equalsValue(
          codeInsertAndUpdateField.getFrontComponent())) {
