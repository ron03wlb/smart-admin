package net.lab1024.sa.support.codegenerator.service.variable.backend;
...
import net.lab1024.sa.support.codegenerator.constant.CodeDeleteEnum;
  ...
  public List<String> getPackageList(
    ...
    packageSet.addAll(
        getJavaBeanImportClass(form).stream()
            .filter(e -> !e.contains("Entity;"))
            .collect(Collectors.toList()));

    // 2、其他包
    if (form.getDeleteInfo().getIsSupportDelete()) {

      CodeDeleteEnum codeDeleteEnum =
          SmartEnumUtil.getEnumByValue(form.getDeleteInfo().getDeleteEnum(), CodeDeleteEnum.class);
      if (codeDeleteEnum == CodeDeleteEnum.BATCH
          || codeDeleteEnum == CodeDeleteEnum.SINGLE_AND_BATCH) {
        // 2、批量删除的话，要导入ValidateList
        packageSet.add(IMPORT_PREFIX + "net.lab1024.sa.base.common.domain.ValidateList;");
      }

      if (codeDeleteEnum == CodeDeleteEnum.SINGLE
          || codeDeleteEnum == CodeDeleteEnum.SINGLE_AND_BATCH) {
        // 3、单个删除的话，要导入 @PathVariable
        packageSet.add(IMPORT_PREFIX + "org.springframework.web.bind.annotation.PathVariable;");
        packageSet.add(IMPORT_PREFIX + "org.springframework.web.bind.annotation.GetMapping;");
      }
    }
    ...
    packageSet.add(
        IMPORT_PREFIX
            ...
            + ".service."
            + CaseFormat.LOWER_CAMEL.to(CaseFormat.UPPER_CAMEL, form.getBasic().getModuleName())
            + "Service;");
