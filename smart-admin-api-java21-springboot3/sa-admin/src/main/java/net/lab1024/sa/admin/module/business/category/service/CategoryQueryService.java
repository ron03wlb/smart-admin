...
public class CategoryQueryService {
  ...
  /** 根据类目id 递归查询该id的所有子类id 递归查询 同时存入缓存 注意：查询出来的集合 不包含传递的父类参数 */
  public List<Long> queryCategorySubId(List<Long> categoryIdList) {
    ...
    Map<Long, List<CategoryEntity>> subTypeMap =
        categoryEntityList.stream().collect(Collectors.groupingBy(CategoryEntity::getCategoryId));
    // 递归查询子类
    List<Long> subCategoryIdList =
        subTypeMap.values().stream()
    ...
    subCategoryIdList.addAll(this.queryCategorySubId(subCategoryIdList));
  ...
  public Map<Long, String> queryFullName(List<Long> categoryIdList) {
    ...
    return categoryIdList.stream()
        .collect(Collectors.toMap(Function.identity(), this::queryFullName));
  }
}
