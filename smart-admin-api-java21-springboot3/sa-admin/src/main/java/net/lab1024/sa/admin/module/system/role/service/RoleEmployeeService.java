...
public class RoleEmployeeService {
  ...
  public ResponseDTO<PageResult<EmployeeVO>> queryEmployee(
    ...
    List<EmployeeVO> employeeList =
        roleEmployeeDao.selectRoleEmployeeByName(page, roleEmployeeQueryForm).stream()
    ...
    if (CollectionUtils.isNotEmpty(departmentIdList)) {
      ...
      Map<Long, String> departmentIdNameMap =
          departmentEntities.stream()
              .collect(
                  Collectors.toMap(
                      DepartmentEntity::getDepartmentId, DepartmentEntity::getDepartmentName));
      employeeList.forEach(
          e -> {
            e.setDepartmentName(
                departmentIdNameMap.getOrDefault(e.getDepartmentId(), StringConst.EMPTY));
          });
    }
    PageResult<EmployeeVO> pageResult =
        SmartPageUtil.convert2PageResult(page, employeeList, EmployeeVO.class);
  ...
  /** 批量删除角色的成员员工 */
  public ResponseDTO<String> batchRemoveRoleEmployee(
      RoleEmployeeUpdateForm roleEmployeeUpdateForm) {
    roleEmployeeDao.batchDeleteEmployeeRole(
        roleEmployeeUpdateForm.getRoleId(), roleEmployeeUpdateForm.getEmployeeIdList());
    return ResponseDTO.ok();
  }
  ...
  public ResponseDTO<String> batchAddRoleEmployee(RoleEmployeeUpdateForm roleEmployeeUpdateForm) {
    ...
    // 数据库里已有的员工id列表
    Set<Long> dbEmployeeIdList =
        roleEmployeeDao.selectEmployeeIdByRoleIdList(Lists.newArrayList(roleId));
    // 从已选择的员工id列表里 过滤数据库里不存在的 即需要添加的员工 id
    Set<Long> addEmployeeIdList =
        selectedEmployeeIdList.stream()
