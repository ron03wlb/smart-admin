...
public class CategoryService {
  ...
  /** 检查是否有未删除的子类目 */
  private boolean hasSubCategory(Long categoryId) {
    List<CategoryEntity> subList = categoryCacheManager.querySubCategory(categoryId);
    return subList.stream().anyMatch(e -> !e.getDeletedFlag());
  }
  ...
  /** 根据父级id 查询所有子类 返回层级树 如果父类id 为空 返回所有类目层级 */
  public ResponseDTO<List<CategoryTreeVO>> queryTree(CategoryTreeQueryForm queryForm) {
    ...
    List<CategoryTreeVO> treeList =
        categoryCacheManager.queryCategoryTree(
            queryForm.getParentId(), queryForm.getCategoryType());
