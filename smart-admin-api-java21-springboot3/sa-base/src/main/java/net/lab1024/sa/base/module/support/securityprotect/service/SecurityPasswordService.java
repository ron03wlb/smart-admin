...
public class SecurityPasswordService {
  ...
  private static final int MIN_ALLOWED_REPEAT_TIMES = 1;

  private static final int MIN_CHANGE_PASSWORD_DAYS = 1;

  @Resource private PasswordLogDao passwordLogDao;
  ...
  public ResponseDTO<String> validatePasswordRepeatTimes(
    ...
    // 密码重复次数小于1 无需校验
    if (level3ProtectConfigService.getRegularChangePasswordNotAllowRepeatTimes()
        < MIN_ALLOWED_REPEAT_TIMES) {
      return ResponseDTO.ok();
    }
    ...
    List<String> oldPasswords =
        passwordLogDao.selectOldPassword(
            ...
            level3ProtectConfigService.getRegularChangePasswordNotAllowRepeatTimes());
    boolean isDuplicate =
        oldPasswords.stream()
            .anyMatch(oldPassword -> ARGON2_PASSWORD_ENCODER.matches(newPassword, oldPassword));
    if (isDuplicate) {
  ...
  public boolean checkNeedChangePassword(Integer userType, Long userId) {
    ...
    if (level3ProtectConfigService.getRegularChangePasswordDays() < MIN_CHANGE_PASSWORD_DAYS) {
      return false;
    }
    ...
    LocalDateTime nextUpdateTime =
        passwordLogEntity
            .getCreateTime()
            .plusDays(level3ProtectConfigService.getRegularChangePasswordDays());
