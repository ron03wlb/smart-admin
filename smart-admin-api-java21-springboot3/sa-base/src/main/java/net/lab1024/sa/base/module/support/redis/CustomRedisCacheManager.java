 ...
 *
 * @author CoderKK
 * @since 2025-08-15 13:01:01
 *     <p>支持格式：{@code cacheName#ttl}，其中 ttl 支持 Spring 的 Duration 格式。 特殊值：{@code -1} 表示永久缓存（永不过期）。
 *     <h3>使用示例：</h3>
 *     <pre>
 * // 10 秒后过期
 ...
 *     <h3>支持的 TTL 单位：</h3>
 *     <ul>
 *       <li>{@code ms} / {@code millis} / {@code milliseconds} - 毫秒
 *       <li>{@code s} / {@code secs} / {@code seconds} - 秒
 *       <li>{@code m} / {@code mins} / {@code minutes} - 分钟
 *       <li>{@code h} / {@code hrs} / {@code hours} - 小时
 *       <li>{@code d} / {@code days} - 天
 *     </ul>
 *     <h3>注意事项：</h3>
 *     <ul>
 *       <li>不写单位默认为毫秒
 *       <li>永久缓存（#-1）不会自动过期，请配合 &#64;CacheEvict 手动清理。
 *       <li>避免对频繁更新的数据使用永久缓存，防止数据陈旧。
 *       <li>cacheName 中的 '#' 只解析第一个，后续字符将作为 TTL 处理。
 *     </ul>
 */
@Slf4j
public class CustomRedisCacheManager extends RedisCacheManager {
  ...
  private static final int PARTS_LENGTH_HAS_TTL = 2;

  private static final String PERMANENT_CACHE_TTL = "-1";
  ...
  private Duration parseTtlFromCacheName(String name) {
    ...
    if (parts.length < PARTS_LENGTH_HAS_TTL) {
    ...
    // 特殊处理：-1 表示永久缓存
    if (PERMANENT_CACHE_TTL.equals(ttlStr)) {
