package net.lab1024.sa.base.mybatis.util;
 ...
 * @since 2020-04-23 20:51:40 Copyright <a href="https://1024lab.net">1024创新实验室</a>
 */
@Slf4j
public class SmartPageUtil {
  ...
  public static Page<?> convert2PageQuery(PageParam pageParam) {
    ...
    for (PageParam.SortItem sortItem : sortItemList) {
      ...
      orderItemList.add(
          sortItem.getIsAsc()
              ? OrderItem.asc(sortItem.getColumn())
              : OrderItem.desc(sortItem.getColumn()));
    }
  ...
  /** 转换为 PageResult 对象 */
  public static <T, E> PageResult<T> convert2PageResult(
      Page<?> page, List<E> sourceList, Class<T> targetClazz) {
  ...
  /** 转换分页结果对象 */
  public static <E, T> PageResult<T> convert2PageResult(
      PageResult<E> pageResult, Class<T> targetClazz) {
