package net.lab1024.sa.oa.notice.dao;
...
public interface NoticeDao extends BaseMapper<NoticeEntity> {
  ...
  String NOTICE_ID = "noticeId";

  // ================================= 数据范围相关 【子表】
  // =================================

  /** 保存可见范围 */
  void insertVisibleRange(
      @Param(NOTICE_ID) Long noticeId,
      @Param("visibleRangeFormList") List<NoticeVisibleRangeForm> visibleRangeFormList);

  /** 删除可见范围 */
  void deleteVisibleRange(@Param(NOTICE_ID) Long noticeId);

  /** 相关可见范围 */
  List<NoticeVisibleRangeVO> queryVisibleRange(@Param(NOTICE_ID) Long noticeId);
  ...
  /** 更新删除状态 */
  void updateDeletedFlag(@Param(NOTICE_ID) Long noticeId);

  // ================================= 通知公告【员工查看】 相关
  // =================================

  /** 查询 员工 查看到的通知公告 */
  List<NoticeEmployeeVO> queryEmployeeNotice(
      Page<?> page,
  ...
  /** 查询 员工 未读的通知公告 */
  List<NoticeEmployeeVO> queryEmployeeNotViewNotice(
      Page<?> page,
      @Param("requestEmployeeId") Long requestEmployeeId,
      @Param("query") NoticeEmployeeQueryForm noticeEmployeeQueryForm,
      @Param("requestEmployeeDepartmentIdList") List<Long> requestEmployeeDepartmentIdList,
  ...
  long viewRecordCount(@Param(NOTICE_ID) Long noticeId, @Param("employeeId") Long employeeId);

  /** 查询通知、公告的 查看记录 */
  List<NoticeViewRecordVO> queryNoticeViewRecordList(
      Page page, @Param("queryForm") NoticeViewRecordQueryForm noticeViewRecordQueryForm);
  ...
  void updateViewRecord(
      @Param(NOTICE_ID) Long noticeId,
      @Param("employeeId") Long requestEmployeeId,
      @Param("ip") String ip,
      @Param("userAgent") String userAgent);
  ...
  void updateViewCount(
      @Param(NOTICE_ID) Long noticeId,
      @Param("pageViewCountIncrement") Integer pageViewCountIncrement,
      @Param("userViewCountIncrement") Integer userViewCountIncrement);
}
