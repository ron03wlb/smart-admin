...
public class DepartmentCacheManager {
  ...
  private final net.lab1024.sa.common.cache.CacheService cacheService;
  ...
  /** 构建父级考点路径 */
  private String buildDepartmentPath(
      DepartmentVO departmentVO, Map<Long, DepartmentVO> departmentMap) {
  ...
  public List<DepartmentTreeVO> buildTree(List<DepartmentVO> voList) {
    ...
    List<DepartmentVO> rootList =
        voList.stream()
            .filter(
                e ->
                    e.getParentId() == null
                        || Objects.equals(e.getParentId(), NumberUtils.LONG_ZERO))
  ...
  /** 构建所有根节点的下级树形结构 返回值为层序遍历结果 [由于departmentDao中listAll给出数据根据Sort降序 所以同一层中Sort值较大的优先遍历] */
  private List<Long> recursiveBuildTree(
      List<DepartmentTreeVO> nodeList, List<DepartmentVO> allDepartmentList) {
  ...
  private List<DepartmentTreeVO> getChildren(Long departmentId, List<DepartmentVO> voList) {
    List<DepartmentVO> childrenEntityList =
        voList.stream()
  ...
  public List<Long> selfAndChildrenIdList(Long departmentId, List<DepartmentVO> voList) {
    ...
    List<Long> childrenIdList =
        children.stream().map(DepartmentTreeVO::getDepartmentId).collect(Collectors.toList());
  ...
  /** 递归查询 */
  public void selfAndChildrenRecursion(
      List<Long> selfAndChildrenIdList, Long departmentId, List<DepartmentVO> voList) {
    List<DepartmentTreeVO> children = this.getChildren(departmentId, voList);
    if (CollectionUtils.isEmpty(children)) {
      return;
    }
