...
import net.lab1024.sa.support.serialnumber.domain.SerialNumberGenerateForm;
...
import net.lab1024.sa.support.serialnumber.service.SerialNumberRecordService;
import net.lab1024.sa.support.serialnumber.service.SerialNumberService;
...
public class AdminSerialNumberController extends SupportBaseController {
  ...
  public ResponseDTO<List<String>> generate(
      @RequestBody @Valid SerialNumberGenerateForm generateForm) {
    SerialNumberIdEnum serialNumberIdEnum =
        SmartEnumUtil.getEnumByValue(generateForm.getSerialNumberId(), SerialNumberIdEnum.class);
    if (null == serialNumberIdEnum) {
      return ResponseDTO.userErrorParam("SerialNumberId，不存在" + generateForm.getSerialNumberId());
    }
    return ResponseDTO.ok(
        serialNumberService.generate(serialNumberIdEnum, generateForm.getCount()));
  }
  ...
  public ResponseDTO<PageResult<SerialNumberRecordEntity>> queryRecord(
      @RequestBody @Valid SerialNumberRecordQueryForm queryForm) {
