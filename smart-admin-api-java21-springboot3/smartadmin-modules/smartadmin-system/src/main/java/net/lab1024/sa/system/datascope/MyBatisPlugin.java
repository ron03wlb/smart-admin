...
import net.lab1024.sa.common.mybatis.DataScopePlugin;
import net.lab1024.sa.system.datascope.domain.DataScopeSqlConfig;
import net.lab1024.sa.system.datascope.service.DataScopeSqlConfigService;
...
public class MyBatisPlugin extends DataScopePlugin {
  ...
  public Object intercept(Invocation invocation) throws Throwable {
    ...
    String path =
        methodStrList.get(methodStrList.size() - 2)
            + "."
            + methodStrList.get(methodStrList.size() - 1);
    ...
    if (sqlConfigDTO != null) {
      ...
      BoundSql newBoundSql =
          copyFromBoundSql(
              mappedStatement, boundSql, this.joinSql(originalSql, paramMap, sqlConfigDTO));
      ParameterMap map = mappedStatement.getParameterMap();
      MappedStatement newMs =
          copyFromMappedStatement(mappedStatement, new BoundSqlSqlSource(newBoundSql), map);
  ...
  private String joinSql(
      String sql, Map<String, Object> paramMap, DataScopeSqlConfig sqlConfigDTO) {
    ...
    int orderIndex = sqlLowerCase.indexOf(order);
    int groupIndex = sqlLowerCase.indexOf(group);
    if (whereIndex > -1) {
  ...
  private MappedStatement copyFromMappedStatement(
      MappedStatement ms, SqlSource newSqlSource, ParameterMap parameterMap) {
