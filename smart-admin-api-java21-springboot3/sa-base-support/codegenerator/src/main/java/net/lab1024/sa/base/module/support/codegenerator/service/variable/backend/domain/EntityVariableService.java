...
public class EntityVariableService extends CodeGenerateBaseVariableService {
  ...
  public List<String> getImportPackageList(List<CodeField> fields) {
    ...
    List<String> result =
        fields.stream()
            .map(e -> getJavaPackageName(e.getJavaType()))
    ...
    // 自动填充注解
    boolean existCreateAndUpdate =
        fields.stream()
            .anyMatch(
                e ->
                    "create_time".equals(e.getColumnName())
                        || "update_time".equals(e.getColumnName()));
    ...
    boolean isExistPrimaryKey =
        fields.stream().anyMatch(e -> e.getPrimaryKeyFlag() != null && e.getPrimaryKeyFlag());
    if (isExistPrimaryKey) {
      result.add("import com.baomidou.mybatisplus.annotation.TableId;");
    }

    // 自增
    boolean isExistAutoIncrease =
        fields.stream().anyMatch(e -> e.getAutoIncreaseFlag() != null && e.getAutoIncreaseFlag());
