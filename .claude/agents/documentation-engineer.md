...
**Reference:** See `.claude/shared/knowledge/documentation-patterns.md` → "Transaction Management Documentation Pattern" for full template with correct/incorrect examples.

**Key rules:** `@Transactional(rollbackFor = Throwable.class)` in Manager layer ONLY. Service orchestrates, Manager handles transactions. Validated by ArchitectureTest.
...
**Reference:** See `.claude/shared/knowledge/documentation-patterns.md` → "Manager Layer Caching Documentation Pattern" for full template with `@Cacheable`/`@CacheEvict` examples.

**Key rules:** Caching annotations (`@Cacheable`, `@CacheEvict`) in Manager layer ONLY. Uses Redisson with Redis backend. Document cache name, TTL, key format, and eviction triggers.
...
**Full scenario templates** with complete code examples are in `.claude/shared/knowledge/documentation-patterns.md` → "Common Documentation Scenarios".

**Available scenarios:**
1. **Document New SmartAdmin REST API** — Complete API reference with cURL, Swagger, error codes
2. **Document Manager Layer Transaction Pattern** — Transaction rules, correct/incorrect examples, ArchitectureTest
3. **Document Vue Component API Integration** — ResponseModel handling, v-privilege, unit tests
4. **Document Permission System Alignment** — Backend/frontend permission matching checklist
5. **Document Pagination Pattern** — End-to-end SmartPageUtil with backend + frontend examples
