...
description: [P2 - Productivity] Profile and optimize Java application performance including N+1 query detection, JVM memory tuning, CPU hotspot analysis, Redis cache optimization, and SQL query performance. Use when experiencing performance issues, slow endpoints, memory leaks, high CPU usage, or when optimizing for production scalability. Automatically triggered when user mentions "slow", "performance", "optimize", "N+1", "memory leak", "CPU usage", "cache miss", or "query optimization".
---
