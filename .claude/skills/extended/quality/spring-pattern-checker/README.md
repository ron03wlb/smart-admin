...
- **[Manager Layer Rules](.agent/foundation/F03-manager-layer.md)** - Transaction and cache management
- **[Architecture Rules](.agent/foundation/F04-architecture-rules.md)** - Layered architecture enforcement
- **[Naming Conventions](.agent/foundation/F01-naming-conventions.md)** - Naming standards

## FAQ

**Q: Why can't I use @Transactional in Service layer?**
A: SmartAdmin enforces explicit transaction boundaries in the Manager layer to prevent hidden transaction nesting and ensure proper rollback scope. See [Manager Layer Rules](.agent/foundation/F03-manager-layer.md).
...
A: No. SmartAdmin prohibits Service→Service calls to prevent tight coupling. If multiple Services are needed, create a Manager to orchestrate them. See [Architecture Rules](.agent/foundation/F04-architecture-rules.md).
