...
1. 冪等性檢查（Idempotency Check）： 檢查該 transactionId 是否已處理過。由於網路抖動，GP 可能會重發相同的請求。如果已處理，直接返回緩存的結果，不可重複扣款 10。
...
- 營運商的責任： 營運商必須具備處理「延遲到達」請求的能力。如果一個 Rollback 請求比原始的 Bet 請求先到達（亂序），系統必須能夠記錄下「該交易若到達則直接取消」的狀態，或者在 Bet 到達時識別出它已被回滾 12。
...
- 贏半/輸半（Half Win / Half Loss）： 這是亞洲盤口（Asian Handicap）特有的邏輯。
    

- 例如：投注 100 元在「讓球 -0.25」，結果為平局。玩家輸掉一半本金（50元），退回一半（50元）。
    
- 計算規則： 在大多數主流系統（如沙巴體育、CMD368）及營運商規則中，此時的 Valid Bet 取決於「實際風險暴露」。通常計算為 50元（即實際輸贏的金額絕對值），而非本金 100 元。這要求 API 回調中必須包含詳細的 outcome 狀態（如 HalfWon, HalfLost）18。
...
- 紅黑對打： 若同時下注紅與黑，Valid Bet 為 0。
    
- 技術實現： GP 的 API（如 Evolution Gaming）會在 Result 封包中傳送詳細的 bet_code 或 transaction_details。營運商的風控引擎需解析這些代碼，計算覆蓋率。部分 GP 會直接提供 coverage 參數，但營運商通常傾向於使用自己的邏輯進行二次驗證 20。
...
- 和局（Tie）： 由於和局的賠率高但出現機率低，且通常會退還莊閒的注金，許多營運商在計算「返水」時會排除和局的投注，或僅計算和局的輸贏金額。
...
- 分錄示例（下注）： 借：玩家現金帳戶 / 貸：未結算注單負債。
    
- 分錄示例（結算贏）： 借：未結算注單負債 / 貸：玩家現金帳戶（本金）；借：收入抵減（GGR）/ 貸：玩家現金帳戶（贏金）。
    
- 分錄示例（結算輸）： 借：未結算注單負債 / 貸：博彩收入（Revenue）。
...
無縫錢包最常見的財務問題是「掉單」——即 GP 與營運商的數據不一致。

1. 三方對帳模型：
    

- 方 1：營運商帳本（Internal Ledger）： 記錄了所有 API 調用的結果。
    
- 方 2：供應商報表（Provider Report）： GP 提供的每日/每小時交易清單。
    
- 方 3：支付網關/銀行（Gateway）： 針對存提款的實際資金流動 35。
...
3. 對帳自動化： 現代系統通常透過 API（如 Pragmatic 的 GetPlayedGames 或 Evolution 的 History API）定期拉取數據，進行逐筆比對（Row-by-Row Matching），自動生成差異報告 3。
...
- 邏輯： Redis.incr(user_daily_turnover, valid_bet). 如果 new_value >= 1000 且 status == incomplete，則觸發獎勵。
...
- **流水進度追蹤與解鎖機制** （符合業界標準 - Pragmatic Play / Evolution Gaming）：
    ...
    > **注意**：過往文檔曾描述「投注時自動解鎖」為錯誤邏輯，已修正為「取款時驗證」。詳見：[流水要求驗證時機設計](seamless_wallet_analysis/11_wagering_requirement_timing_and_traceability.md)
