...
> **目標讀者**: 高階主管、合規官、產品經理
> **相關架構**: [Financial_Implementation.md](../../architecture/02_Finance_Service/04_Financial_Implementation.md)
> **最後同步**: 2026-02-09
>
...
| 錢包類型 | 用途 | 業務規則 | 記錄數/玩家 | Phase |
|---------|------|---------|------------|-------|
| **CASH** | 真錢餘額，用於下注和提款 | 所有財務操作的主要錢包 | 1 筆（必有） | P1 |
| **BONUS** | 受有效投注額要求限制的促銷資金 | 在達到有效投注額條件前無法提款。主表記匯總餘額，擴展表（`t_wallet_bonus_ext`）記每筆活動明細 | 0~1 筆匯總 + 0~N 筆明細 | P1 |
| **CREDIT** | 代理信用額度，僅限信用模式玩家 | 信用額度上限、結算週期、所屬代理。擴展表（`t_wallet_credit_ext`）承載專屬欄位 | 0~1 筆 | P2+ |

> **LOCKED 不是錢包類型**：凍結資金是 CASH 錢包上的「扣住機制」（資金始終留在 CASH 錢包中），透過 `t_wallet.locked_amount`（匯總欄位）+ `t_wallet_lock`（明細表）的雙軌設計實現。詳見 → [Financial_Implementation.md §2.1](../../architecture/02_Finance_Service/04_Financial_Implementation.md)

**預設扣款優先順序**：BONUS → CASH → CREDIT（可按遊戲配置覆蓋）
...
→ **[技術實施](../../architecture/02_Finance_Service/04_Financial_Implementation.md#concurrency-control)** - 原子性、冪等性、最終一致性機制
...
→ **[簽名演算法詳情](../../architecture/02_Finance_Service/04_Financial_Implementation.md#signature-verification)** - HMAC-SHA256 生成流程、參數排序、Base64 編碼
...
→ **[SAGA 模式實施](../../architecture/02_Finance_Service/04_Financial_Implementation.md#saga-compensation)** - 技術流程圖、補償步驟、分散式交易處理
...
→ **[快取/資料庫同步策略](../../architecture/02_Finance_Service/03_Seamless_Wallet_Technical.md#cache-strategy)**
- 三方投注對帳達到 100% 準確性
- 差異告警及時發送
- 對帳報表自動生成
- 差異解決流程完整記錄於審計日誌
