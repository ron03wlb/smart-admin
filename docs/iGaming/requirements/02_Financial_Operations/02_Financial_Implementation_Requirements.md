...
> **LOCKED 不是錢包類型**：凍結資金是 CASH 錢包上的「扣住機制」（資金始終留在 CASH 錢包中），透過 `t_wallet.locked_amount`（匯總欄位）+ `t_wallet_lock`（明細表）的雙軌設計實現。詳見 → [Financial_Implementation.md §2.1](../../architecture/02_Finance_Service/Financial_Implementation.md)
