...
→ **[Smart Routing Algorithm](../../architecture/02_Finance_Service/Payment_Gateway_Technical.md#smart-routing-algorithm)** - 分數計算公式、加權排序實作、即時 PSP 選擇邏輯
...
→ **[PSP Webhook Integration](../../architecture/02_Finance_Service/Payment_Gateway_Technical.md#psp-webhook-integration)** - 簽章驗證（HMAC-SHA256）、回調處理、資料庫更新
...
→ **[Auto Reconciliation Implementation](../../architecture/02_Finance_Service/Payment_Gateway_Technical.md#auto-reconciliation)** - 排程任務配置、PSP API 整合、SQL 查詢、狀態同步邏輯
...
→ **[Performance Monitoring](../../architecture/02_Finance_Service/Payment_Gateway_Technical.md#performance-monitoring)** - Prometheus 指標配置、Grafana 儀表板、警報規則（PagerDuty、Slack）
...
→ **[Security Implementation](../../architecture/02_Finance_Service/Payment_Gateway_Technical.md#security-implementation)** - TLS 1.2+ 配置、HashiCorp Vault 整合、API 金鑰輪替政策
...
→ **[3DS Integration](../../architecture/02_Finance_Service/Payment_Gateway_Technical.md#3ds-integration)** - 3DS 2.0 流程實作、SCA 豁免邏輯、PSD2 合規驗證
...
→ **[Payment Gateway API Architecture](../../architecture/02_Finance_Service/Payment_Gateway_API.md)** - PSP 適配器實作、webhook 處理、智慧路由演算法、對帳自動化、HikariCP 調校、Prometheus 監控
