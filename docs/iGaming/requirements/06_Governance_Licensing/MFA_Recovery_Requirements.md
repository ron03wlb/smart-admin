...
→ **[MFA Session 存儲實作](../../architecture/06_Platform_Core/MFA_Login_Recovery_Technical.md#mfa-session-storage)** - Redis 存儲配置、5 分鐘 TTL、Session 清理機制
...
→ **[Trusted Device Token 實作](../../architecture/06_Platform_Core/MFA_Login_Recovery_Technical.md#trusted-device-token)** - Cookie 配置（HttpOnly、Secure、SameSite=Strict）、SHA256 Token 生成、三重綁定實作
...
→ **[TOTP Secret 和 QR Code 生成](../../architecture/06_Platform_Core/MFA_Login_Recovery_Technical.md#totp-registration)** - Secret 生成算法、QR Code 編碼、安全顯示實作
...
→ **[Backup Code 存儲實作](../../architecture/06_Platform_Core/MFA_Login_Recovery_Technical.md#backup-codes)** - 加密算法（AES-256-GCM）、安全生成、存儲格式
...
→ **[MFA Recovery Implementation](../../architecture/06_Platform_Core/MFA_Recovery_Implementation.md)** - 兩階段登入序列圖、MFA Session 存儲（Redis + TTL）、信任設備 Token 生成（SHA256）、Cookie 配置（HttpOnly/Secure/SameSite）和 TOTP QR Code 生成
