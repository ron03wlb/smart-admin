...
**優化後架構** (基於 [07-07 §5](./07-07_Performance_Optimization.md#5-數據分層存儲)):
```
...
**資源配置** (基於 [07-08 §7](./07-08_Stream_Processing_Architecture.md#7-部署架構)):
```yaml
...
**優化後** (基於 [07-08 §3](./07-08_Stream_Processing_Architecture.md#3-flink-sql-實時-olap) Flink 分流 OLAP 查詢):
```
...
**優化後** (基於 [07-09 §2](./07-09_Caching_Strategy.md#2-jetcache-多級緩存架構) JetCache L1 減少 Redis 訪問):
```
Redis Cluster (3 master + 3 replica):
- r5.xlarge × 6 = $290.4/月
- 內存使用率: 70% (高效)
