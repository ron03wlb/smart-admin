...
| [15-09_Self_Exclusion_Reconciliation.md](15-09_Self_Exclusion_Reconciliation.md) | 自我排除同步對帳 (Gamstop) | P0 |
...
## 效果評估指標 (RG Effectiveness Metrics)

### 核心 KPIs

負責任博彩工具的效果需要通過量化指標進行評估，以持續改進玩家保護措施。

| 指標類別 | 指標名稱 | 計算方式 | 目標值 | 說明 |
|---------|---------|---------|--------|------|
| **工具使用率** | 限額設定率 | 設定限額玩家 / 活躍玩家 | > 15% | 玩家主動使用保護工具 |
| | 現實檢查配置率 | 配置 RC 玩家 / 活躍玩家 | > 20% | |
| | 自我排除率 | 月新增排除 / 活躍玩家 | 監控趨勢 | 過高可能表示問題 |
| **工具效果** | 限額觸發後繼續率 | 達限額後繼續嘗試存款 / 達限額事件 | < 30% | 低值表示限額有效 |
| | 現實檢查後停止率 | RC 彈窗後停止 / RC 彈窗總數 | > 20% | 高值表示 RC 有效 |
| | 排除後嘗試重新註冊率 | 排除後新註冊嘗試 / 總排除 | < 5% | 排除機制效果 |
| **風險識別** | 高風險玩家識別率 | 識別 / 事後確認高風險 | > 80% | 模型準確率 |
| | 早期介入成功率 | 介入後改善 / 總介入 | > 40% | 主動介入效果 |
| **財務影響** | 問題博彩相關退款率 | 問題博彩退款 / 總營收 | < 0.5% | 財務風險控制 |
| | 高風險玩家 GGR 佔比 | 高風險玩家 GGR / 總 GGR | < 5% | 避免依賴高風險玩家 |
...
| 報告類型 | 頻率 | 接收者 | 內容 |
|---------|------|--------|------|
| **內部儀表板** | 實時 | RG Team | 所有 KPIs |
| **管理層報告** | 月度 | C-Level | 摘要 + 趨勢 |
| **監管報告** | 季度 | UKGC/MGA | 合規指標 |
| **年度審計** | 年度 | 外部審計 | 完整評估 |
...
rg_tool_usage_rate:
  type: gauge
  labels: [tool_type, jurisdiction]
  description: "負責任博彩工具使用率"

rg_intervention_success:
  type: counter
  labels: [intervention_type, outcome]
  description: "主動介入結果統計"

rg_high_risk_player_ratio:
  type: gauge
  labels: [risk_level]
  description: "各風險等級玩家佔比"
```
